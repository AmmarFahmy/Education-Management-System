...

{
	...
	public function teacher_info_instudent()
	{
		$teacher_info_instudent_select = "select * from teacher_info";
	...
	public function add_marks($student_name, $student_grade, $Math, $Computer, $English, $Science, $Sinhala, $Tamil, $Lib)
	{
		$add_marks = "insert into result(student_name,student_grade,Math,Computer,English,Science,Sinhala,Tamil,Lib) values('$student_name','$student_grade','$Math','$Computer','$English','$Science','$Sinhala','$Tamil','$Lib')";
		$add_marks_run = $this->connectdb->query($add_marks);
		return $add_marks_run;
	}
	...
	public function result_info()
	{
		...
		$results_info_admin = "select * from result";
		$results_info_admin_run = $this->connectdb->query($results_info_admin);
		return $results_info_admin_run;
	}

	////////// add subjects ////////////////////////
	public function add_subjects($subject_name, $t_fullname, $t_email, $grade, $time)
	{
		$add_subjects = "insert into subjects_info(subject_name,t_fullname,t_email,grade,time) values('$subject_name','$t_fullname','$t_email','$grade','$time')";
		$add_sub_run = $this->connectdb->query($add_subjects);
		return $add_sub_run;
	}

	////////// add addmission ////////////////////////
	public function add_addmission($name, $dob, $fname, $gender, $mobile, $address, $district, $state)
	{
		$add_addmission = "insert into admission_form(name,dob,fname,gender,mobile,address,district,state) values('$name','$dob','$fname','$gender','$mobile','$address','$district','$state')";
		$add_subm_run = $this->connectdb->query($add_addmission);
		return $add_subm_run;
	}


	///////////////// update st info from admin/////////////
	public function st_info_display_admin()
	{
		$st_info_admin = "select * from st_info";
		$st_info_admin_run = $this->connectdb->query($st_info_admin);
		return $st_info_admin_run;
	}

	public function edit_st_id($st_id)
	{
		$edit_st_id = "select * from st_info where st_id='$st_id'";
		$edit_st_id_run = $this->connectdb->query($edit_st_id);
		return $edit_st_id_run;
	}

	public function update_st_info($up_fullname, $up_grade, $up_section, $up_roll_no, $up_dob, $up_address, $up_district, $up_gender, $up_father, $up_mother, $up_contact, $st_id)
	{
		$update_st_info_select = "update st_info set st_fullname='$up_fullname',st_grade='$up_grade',st_section='$up_section',roll_no='$up_roll_no',st_dob='$up_dob',st_address='$up_address',st_district='$up_district',st_gender='$up_gender',st_father='$up_father',st_mother='$up_mother',st_parents_contact='$up_contact' where st_id='$st_id'";
		$update_st_info_run = $this->connectdb->query($update_st_info_select);
		return $update_st_info_run;
	}

	public function marks_info_display_admin()
	{
		$marks_info_admin = "select * from result";
		$marks_info_admin_run = $this->connectdb->query($marks_info_admin);
		return $marks_info_admin_run;
	}

	public function edit_marksid($marks_id)
	{
		$edit_marksid = "select * from result where id='$marks_id'";
		$edit_marksid_run = $this->connectdb->query($edit_marksid);
		return $edit_marksid_run;
	}

	public function update_marks_info($up_fullname, $up_grade, $up_math, $up_computer, $up_english, $up_science, $up_sinhala, $up_tamil, $up_lib, $marks_id)
	{
		$update_marks_info_select = "update result set student_name='$up_fullname',student_grade='$up_grade',Math='$up_math',Computer='$up_computer',English='$up_english',Science='$up_science',Sinhala='$up_sinhala',Tamil='$up_tamil',Lib='$up_lib' where id='$marks_id'";
		$update_marks_info_run = $this->connectdb->query($update_marks_info_select);
		return $update_marks_info_run;
	}

	public function subjects_info_display_admin()
	{
		$subject_info_admin = "select * from subjects_info";
		$subject_info_admin_run = $this->connectdb->query($subject_info_admin);
		return $subject_info_admin_run;
	...
	public function edit_subid($sub_id)
	{
		$edit_subid = "select * from subjects_info where id='$sub_id'";
		$edit_subid_run = $this->connectdb->query($edit_subid);
		return $edit_subid_run;
	}

	public function update_sub_info($up_subname, $up_t_fullname, $up_t_email, $up_grade, $up_time, $sub_id)
	{
		$update_sub_info_select = "update subjects_info set subject_name='$up_subname',t_fullname='$up_t_fullname',t_email='$up_t_email',grade='$up_grade',time='$up_time' where id='$sub_id'";
		$update_sub_info_run = $this->connectdb->query($update_sub_info_select);
		return $update_sub_info_run;
	}

	public function parent_info_display_admin()
	{
		$parent_info_admin = "select * from st_info";
		$parent_info_admin_run = $this->connectdb->query($parent_info_admin);
		return $parent_info_admin_run;
	}

	public function admission_info_display_admin()
	{
		$admission_info_admin = "select * from admission_form";
		$admission_info_admin_run = $this->connectdb->query($admission_info_admin);
		return $admission_info_admin_run;
	}

	public function contact_info_display_admin()
	{
		$contact_info_admin = "select * from contact_form";
		$contact_info_admin_run = $this->connectdb->query($contact_info_admin);
		return $contact_info_admin_run;
	}
}
$ravi = new project2;
